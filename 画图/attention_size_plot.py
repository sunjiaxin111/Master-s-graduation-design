...
# 相关数据
x = [2, 4, 6, 8]
criteo_logloss = [0.46963417, 0.46586333, 0.49111222, 0.48238528]
avazu_logloss = [0.39806155, 0.39849626, 0.44128154, 0.43623803]
criteo_auc = [0.75490938, 0.75614338, 0.75204842, 0.75120420]
avazu_auc = [0.76528282, 0.76487490, 0.75984649, 0.76213670]
...
for a, b in zip(x, avazu_logloss):
    plt.text(a, b + 0.0001, '%.4f' % b, ha='center', va='bottom', fontsize=11)
plt.xlabel("注意力网络的隐层神经元个数attention_size")  # X轴标签
plt.ylabel("logloss")  # Y轴标签
plt.title("不同attention_size下AFFM模型的logloss")  # 图标题
plt.xticks(x, x)
...
plt.xlabel("注意力网络的隐层神经元个数attention_size")  # X轴标签
plt.ylabel("auc")  # Y轴标签
plt.title("不同attention_size下AFFM模型的auc")  # 图标题
plt.xticks(x, x)
